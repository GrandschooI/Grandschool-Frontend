...
const WebsitesTopic: React.FC<propsType> = ({websitesData}) => {
    return (
        <>
            <h2 className={s.websiteTopicName}>{websitesData.category}</h2>
            <ul className={s.websitesList}>
                {websitesData.websites.map((el: any) => {
                    return (
                        <li className={s.websitesItem} key={el.id}>
                            ...
                            <a href={el.link} target='_blank' rel="noreferrer">{el.name}</a> {/*<span>{el.description}</span>*/}
                        </li>
                    )
                })}
            </ul>
